...
async fn main() -> Result<()> {
    ...
    match client.get_recent_scrobbles(1).await {
        Ok(recent_scrobbles) => {
            ...
            if let Some(beatles_track) = recent_scrobbles.iter().find(|s| s.artist == artist) {
                println!(
                    "\n🎵 Found Beatles track in recent scrobbles: '{}'",
                    beatles_track.name
                );
                ...
                if regex.is_match(&beatles_track.name) {
                    ...
                    if let Some(timestamp) = beatles_track.timestamp {
                        ...
                        match edit_context
                            .execute_edit_with_real_data(&mut client, clean_name.clone(), None)
                            .await
                        {
                            Ok(true) => {
    ...
    match client
        .find_recent_scrobble_for_track("Yesterday", "The Beatles", 3)
        .await
    {
        Ok(Some(scrobble)) => {
