...
async fn main() -> Result<()> {
    ...
    match client.get_recent_scrobbles(1).await {
        Ok(recent_scrobbles) => {
            ...
            if let Some(test_track) = recent_scrobbles.iter()
                .find(|s| s.timestamp.is_some()) {
                
                println!("\n🎵 Testing edit form loading with: '{}' by '{}'", 
                    test_track.name, test_track.artist);
                
                match client.load_edit_form_values(&test_track.name, &test_track.artist).await {
                    Ok(edit_data) => {
                        ...
                        test_edit.track_name = format!("{} [TEST EDIT]", test_edit.track_name_original);
