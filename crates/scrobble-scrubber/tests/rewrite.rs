...
fn handles_queen_track_with_chris_thile_rules() {
    ...
    assert_eq!(
        edit.album_artist_name, None,
        "Album artist stays None (unknown) — never fabricated from the artist, and no Chris Thile rules should apply"
    );
...
fn should_have_consistent_apply_and_matches_behavior() {
    ...
    assert_eq!(
        edit.album_artist_name, None,
        "Album artist stays None (unknown, never fabricated) when the rule doesn't match"
    );
...
#[test_log::test]
fn sd_rule_serde_round_trip_ignores_regex_cache() {
    let rule = SdRule::new(r"(?P<a>.+) - Remaster", "${a}").with_flags("i");
    // Warm the cache before serializing.
    assert!(rule.matches("Song - Remaster").unwrap());
    let json = serde_json::to_string(&rule).unwrap();
    assert!(!json.contains("compiled"), "cache field must not serialize");
    let back: SdRule = serde_json::from_str(&json).unwrap();
    assert_eq!(back, rule);
    assert_eq!(back.apply("Song - Remaster").unwrap(), "Song");
    // Clones carry equal behavior too.
    let cloned = back.clone();
    assert_eq!(cloned.apply("Song - Remaster").unwrap(), "Song");
}
