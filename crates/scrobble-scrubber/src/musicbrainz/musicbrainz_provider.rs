use crate::filters::{ReleaseFilterConfig, ReleaseFilterType};
use crate::provider::{ScrubActionProvider, SuggestionWithContext};
...
impl ScrubActionProvider for MusicBrainzScrubActionProvider {
    type Error = crate::provider::ActionProviderError;
...
mod tests {
    ...
    async fn should_implement_provider_interface_correctly() {
        let provider = MusicBrainzScrubActionProvider::default();
        assert_eq!(provider.provider_name(), "musicbrainz");
